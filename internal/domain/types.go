// Package domain holds the vocabulary of the product: a gist, its files, its author,
// and the inputs that change them.
//
// It has no behaviour and no dependencies, so every layer can name the same things
// without importing another layer to do it. The API layer used to say domain.Gist,
// which made the shape of a gist the data layer's property rather than the
// product's.
package domain
...
// blob_retention_within_ceiling constraint enforces it; this constant exists so a
// caller can reject an out-of-range request with a clear error instead of letting
// the database clamp it silently.
